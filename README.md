...
With whis fully functional applications setup, I manually run the steps to simulate the application switch while performing an upgrade:
...
Afterwards, I ported this solution to a full docker-compose schema, having the nginx proxy included as another service container (whichs grants the same benefits of containerizing apps, specially for future portability and removes the need of having previously installed nginx in the target machine).
...
I manually run this solution, step by step, and the zero downtime was achieved again. Requests where served by the app #2 container while app #1 container was being upgrade, and viceversa.
